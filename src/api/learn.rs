...
pub async fn learn_command(
    ...
    if !params.force {
        if let Ok(Some(_)) = state.db.get_command(command, "local") {
            return Ok(Json(LearnResponse {
                ...
                source: "".to_string(),
                message: format!("Command '{}' already learned. Use force=true to re-learn.", command),
            }));
        }
    }
    ...
    let (content, source) = if params.man {
        learn::get_man_page(command)
            .or_else(|_| learn::get_help_output(command))
    } else {
        learn::get_help_output(command)
            .or_else(|_| learn::get_man_page(command))
    ...
    // Save to database
    state.db.save_command(&cmd).map_err(|e| Json(ErrorResponse {
        error: format!("Failed to save command: {}", e),
    }))?;

    // Index for search
    let mut search = state.search.write().await;
    search.index_single_command(&cmd).map_err(|e| Json(ErrorResponse {
